...
use Drupal\node\Entity\Node;
class PropertyForm extends FormBase {
  /**
   * {@inheritdoc}
   */
...
{
  $getPropertyIdFromQuery = $this->getRequest()->query->get('pid');
  if(!$getPropertyIdFromQuery){
    $pid = '00000';
    \Drupal::messenger()->addMessage(t("PROPERTY NOT FOUND!"), 'error');

  }else {
    $pid = $getPropertyIdFromQuery;
  }

 //get current date
  $loadCurrentDateFromPId = $this->getCurrentDateFromPvId($pv_id);
